...
ui <- dashboardPage(
    skin = "blue",
    dashboardHeader(
        title = "Epidemiological and evolutionary dynamics of SARS-CoV-2",
        titleWidth = 560
    ),
    dashboardSidebar(
        width = 240,
...
server <- function(input, output, session) {
    ...
    newres <- eventReactive(c(input$go1), {  
        ...
        withProgress(message = "Simulating model..", value = 1.0, {             
            ...
            R0.list <- rep(R0.list.a, length = length(time))
            
            plots.doses.both(time, R0.list, input)
        })
    }, ignoreNULL = FALSE)

   #     output$plot11 <- renderPlot({
   #         if(input$plotVaccine == "TRUE"){
   #         plots <- newres()
